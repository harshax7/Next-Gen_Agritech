...
YOLO_CONFIDENCE_THRESHOLD = 0.6  # Back to original 0.5 since single image works
DEBUG_MODE = True  # Set to True to see detection details

# Frame preprocessing to match single image prediction
PREPROCESS_FRAME = True  # Set to True to match single image preprocessing
TARGET_FRAME_SIZE = None  # Set to (width, height) if you want to resize, None for original

# Size filtering (helps reject hands/cloth)
MIN_TOMATO_SIZE = 30    # Minimum width/height in pixels
MAX_TOMATO_SIZE = 400   # Maximum width/height in pixels
USE_SIZE_FILTER = False  # Disabled since YOLO should work correctly now
...
DISPLAY_HEIGHT = 720
FPS_DISPLAY = True
USE_COLORED_BOXES = True  # Set to False to use single color for all boxes
...
DEFAULT_COLOR = (255, 255, 255)  # White (used if USE_COLORED_BOXES = False)
...
class TomatoDetectionSystem:
    ...
    def _load_yolo_model(self):
        ...
        print(f"[YOLO] Available classes: {class_names}")
        
        # Identify which class is tomato
        self.tomato_class_names = [name for name in class_names if 'tomato' in name.lower()]
        print(f"[YOLO] Tomato classes identified: {self.tomato_class_names}")
        
        if not self.tomato_class_names:
            print("[YOLO] WARNING: No 'tomato' class found in model!")
            print(f"[YOLO] Please verify your model has a class containing 'tomato'")
            print(f"[YOLO] Current classes: {class_names}")
    ...
    def detect_tomatoes(self, frame):
        ...
        Uses the same prediction method as single image prediction
        ...
        """
        # Preprocess frame to match single image prediction if enabled
        ...
        if PREPROCESS_FRAME:
            ...
            # Optional: Resize frame if target size is specified
            if TARGET_FRAME_SIZE is not None:
                processed_frame = cv2.resize(processed_frame, TARGET_FRAME_SIZE)
        
        # Run YOLO detection - exactly as in predict_tomato() function
        # Note: YOLO handles BGR images from OpenCV directly
        results = self.yolo_model(processed_frame, device=self.device, verbose=False)
        ...
        # Process YOLO results - same as predict_tomato() function
        for result in results:
            if result.boxes is not None:
                ...
                for box in result.boxes:
                    ...
                    if DEBUG_MODE:
                        print(f"[YOLO DEBUG] Detected: '{class_name}' | Confidence: {confidence:.3f}")
                    ...
                    if confidence >= YOLO_CONFIDENCE_THRESHOLD:
                        ...
                        # Check if this detection is a tomato class
                        # YOLO should only have "tomato" class
                        is_tomato = class_name.lower() in [name.lower() for name in self.tomato_class_names]
                        
                        if DEBUG_MODE:
                            print(f"  Class name: '{class_name}' | Is tomato: {is_tomato}")
                            if not is_tomato:
                                print(f"  -> Not in tomato classes: {self.tomato_class_names}")
                        
                        # Apply size filter if enabled
                        size_valid = True
                        if USE_SIZE_FILTER and is_tomato:
                            if width < MIN_TOMATO_SIZE or height < MIN_TOMATO_SIZE:
                                size_valid = False
                                if DEBUG_MODE:
                                    print(f"  ✗ Too small: {width:.0f}x{height:.0f}px (min: {MIN_TOMATO_SIZE}px)")
                            elif width > MAX_TOMATO_SIZE or height > MAX_TOMATO_SIZE:
                                size_valid = False
                                if DEBUG_MODE:
                                    print(f"  ✗ Too large: {width:.0f}x{height:.0f}px (max: {MAX_TOMATO_SIZE}px)")
                        
                        if not size_valid:
                            continue
                        
                        if DEBUG_MODE and is_tomato:
                            print(f"  ✓ Valid tomato detected: {class_name} ({width:.0f}x{height:.0f}px)")
                        elif DEBUG_MODE and not is_tomato:
                            print(f"  ✗ Not a tomato: {class_name}")
                        ...
                        print(f"  -> Filtered out (below threshold {YOLO_CONFIDENCE_THRESHOLD})")
            else:
                if DEBUG_MODE:
                    print("[YOLO] No detections in this frame")
    ...
    def process_frame(self, frame):
        ...
        for detection in detections:
            ...
            if detection['is_tomato']:
                ...
                if cropped_tomato.size > 0:
                    ...
                    if ripeness:
                        ...
                        if ripeness == 'Ripe':
                            ...
                            color = COLOR_RIPE if USE_COLORED_BOXES else DEFAULT_COLOR
                        else:
                            label = f"Unripe Tomato {confidence:.2%}"
                            color = COLOR_UNRIPE if USE_COLORED_BOXES else DEFAULT_COLOR
                ...
                color = COLOR_NOT_TOMATO if USE_COLORED_BOXES else DEFAULT_COLOR
                self._draw_detection(frame, x1, y1, x2, y2, label, color)
    ...
    try:
        while True:
            ...
            if key == ord('q'):
                ...
                YOLO_CONFIDENCE_THRESHOLD = min(0.95, YOLO_CONFIDENCE_THRESHOLD + 0.05)
                print(f"Confidence threshold: {YOLO_CONFIDENCE_THRESHOLD:.2f}")
            elif key == ord('-') or key == ord('_'):
                # Decrease confidence threshold
                YOLO_CONFIDENCE_THRESHOLD = max(0.1, YOLO_CONFIDENCE_THRESHOLD - 0.05)
                print(f"Confidence threshold: {YOLO_CONFIDENCE_THRESHOLD:.2f}")
