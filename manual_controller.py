"""
Manual Keyboard Controller — CALANDIS 6-DOF Arm
================================================
...
# ============================================================================
# JOINT CONFIGURATION
# Based on CALANDIS 6-DOF kit with PCA9685
# Channels 0-5 in order from base to gripper
# ============================================================================
...
JOINTS = {
    ...
    'wrist_ud':  {'channel': 3, 'min': 30, 'max': 120, 'home': 90, 'step': 3},
    'wrist_rot': {'channel': 4, 'min': 30, 'max': 120, 'home': 90, 'step': 3},
    'gripper':   {'channel': 5, 'min': 0,  'max': 90,  'home': 0,  'step': 5},
...
KEY_MAP = {
    ...
    ord('r'): ('wrist_ud',  +1),   # wrist up
    ord('f'): ('wrist_ud',  -1),   # wrist down
    ord('t'): ('wrist_rot', +1),   # wrist rotate one way
    ord('g'): ('wrist_rot', -1),   # wrist rotate other way
    ord('y'): ('gripper',   -1),   # gripper open (decrease angle)
    ord('h'): ('gripper',   +1),   # gripper close (increase angle)
}
...
class ServoController:
    ...
    def __init__(self):
        try:
            ...
            # Set actuation range for all MG996R servos
            ...
            print("         Check I2C wiring and make sure I2C is enabled on Pi")
    ...
    def print_angles(self):
        """Print current angles of all joints — useful for noting home position."""
...
class ManualController:
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                # Draw controls overlay on frame
                self._draw_overlay(frame)
                cv2.imshow('Manual Control — CALANDIS 6-DOF Arm', frame)

                # Read key (1ms wait — non-blocking)
                ...
                elif key == ord('p'):  # P — print current angles
            ...
            print("Use them to fill in JOINTS home angles in arm_controller.py\n")
    ...
    def _draw_overlay(self, frame):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, FRAME_HEIGHT - 180),
        ...
        controls = [
            ...
            "R/F:Wrist  T/G:WristRot  Y/H:Gripper",
            "SPACE:Home  P:PrintAngles  ESC:Stop",
        ]
        y = FRAME_HEIGHT - 165
        ...
        # Divider line
        cv2.line(frame, (0, FRAME_HEIGHT - 120),
                 (FRAME_WIDTH, FRAME_HEIGHT - 120), (60, 60, 60), 1)

        # Joint angles
        y = FRAME_HEIGHT - 105
        col = 10
        for i, (name, angle) in enumerate(self.servos.current_angles.items()):
            color = (0, 255, 180)
            # Turn red if near limit
            cfg = JOINTS[name]
            if angle <= cfg['min'] + 5 or angle >= cfg['max'] - 5:
                color = (0, 80, 255)
            cv2.putText(frame, f"{name}: {angle:.0f}deg",
                        (col, y), cv2.FONT_HERSHEY_SIMPLEX,
                        0.42, color, 1)
            y += 18
            if i == 2:  # second column after 3 joints
                y = FRAME_HEIGHT - 105
                col = 330

    def _print_controls(self):
        print("=" * 50)
        print("  CALANDIS 6-DOF ARM — MANUAL CONTROL")
        print("=" * 50)
        print("  W / S      →  Base rotate")
        print("  A / D      →  Shoulder up/down")
        print("  Q / E      →  Elbow extend/retract")
        print("  R / F      →  Wrist up/down")
        print("  T / G      →  Wrist rotation")
